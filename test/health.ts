 ...
 * reason the metrics tests use one: these are Express routes, and a fake would
...
describe('Health', () => {
  ...
  before(async () => {
    writable = await mkdtemp(join(tmpdir(), 'omfg-health-'));
  });
  ...
  describe('/healthz', () => {
    it('reports ok', async () => {
      ...
      assert.partialDeepStrictEqual(body, {status: 'ok'});
    });
  ...
  describe('ensureDataDirs', () => {
    it('creates the data dirs, and readiness then passes', async () => {
      const created = join(writable, 'nested', 'cache');
    ...
    it('does not throw when a dir cannot be created', async () => {
      ...
      await ensureDataDirs([join(blocked, 'cache')]);
    });
  });
});
